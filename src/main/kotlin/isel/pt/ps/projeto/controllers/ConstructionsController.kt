...
class ConstructionsController(private val constructionService: ConstructionsService) {
    @GetMapping("/{oid}")
    fun getConstruction(@PathVariable oid: Int): ResponseEntity<*> {
        val construction = constructionService.getConstruction(oid)
        return ResponseEntity.status(200).body(ConstructionOutputModel(
            construction.oid,
            construction.nome,
    ...
    fun getConstructionsUsers(@PathVariable oid: Int): ResponseEntity<*>{
